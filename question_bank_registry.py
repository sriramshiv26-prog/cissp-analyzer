...
class QuestionBankRegistry:
    ...
    def find_matching_question_banks(
        ...
        for answer_file in answer_filenames:
            answer_normalized = answer_file.lower().replace(".json", "").replace(".xlsx", "").replace(".xls", "")
            ...
            for pdf_name, pdf_entry in registry["question_banks"].items():
                ...
                similarity = SequenceMatcher(None, answer_normalized, pdf_normalized).ratio()
...
def print_find_matches_wizard(batch_dir: str):
    ...
    if not matches:
        ...
        print(f"  Suggestions:")
        print(f"    1. Register question banks first: python3 question_bank_registry.py --register july12")

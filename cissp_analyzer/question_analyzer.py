...
class QuestionAnalyzer:
    ...
    def extract_question_type(question_text: str) -> str:
        ...
        if re.search(
            r"\b(not|except|all are|all of the)\b.*\btrue\b", text_lower
        ) or re.search(r"\b(which|what).*\b(not|except)\b", text_lower):
            return "Exception"
        ...
        if len(question_text) > 200 or re.search(
            r"\b[A-Z][a-z]+\s+(is|has|was)\b", question_text
        ):
            return "Scenario"
    ...
    def extract_difficulty(question_text: str, options: Dict[str, str]) -> str:
        ...
        tech_terms = len(
            re.findall(
                r"\b(algorithm|protocol|cipher|biometric|kerberos|pki)\b",
                question_text.lower(),
            )
        )
    ...
    def analyze(question_num: int, question_text: str, options: Dict[str, str]) -> Dict:
        ...
        return {
            ...
            "subtopic": QuestionAnalyzer.extract_topic(
                question_text
            ),  # Can refine later

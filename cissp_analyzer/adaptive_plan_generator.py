...
class AdaptivePlanGenerator:
    ...
    def generate_sheet(self, current_exam: Dict, previous_exam: Optional[Dict] = None):
        ...
        ranked_domains = self.calculator.rank_domains_by_priority(
            current_exam, previous_exam
        )
    ...
    def _add_priority_section(
        self, ws, domain_dict: Dict, priority_num: int, start_row: int
    ) -> int:
        """
        ...
        Args:
            ...
            domain_dict: Dict with domain, current_accuracy, previous_accuracy, momentum
        ...
        if previous_accuracy is not None:
            ...
            momentum_info = f" | Previous: {previous_pct}%"
            momentum_info += f" | Momentum: {momentum_direction} ({momentum_pct:+d}%)"
            status_text += momentum_info
    ...
    def _generate_focus_areas(self, domain_name: str, accuracy: float) -> List[str]:
        ...
        if accuracy < 0.40:
            ...
            focus_areas.append(
                "Practice basic question types before advanced scenarios"
            )
        elif accuracy < 0.55:
        ...
        focus_areas.append(
            f"Target: Increase accuracy from {round(accuracy*100)}% to 85%+"
        )

        return focus_areas

    def _add_strengths_section(
        self, ws, ranked_domains: List[Dict], start_row: int
    ) -> int:
        """
        ...
        high_accuracy_domains = [
            d for d in ranked_domains if d.get("current_accuracy", 0.0) >= 0.80
        ]
        ...
        if high_accuracy_domains:
            ...
            for domain in sorted(
                high_accuracy_domains,
                key=lambda x: x.get("current_accuracy", 0.0),
                reverse=True,
            ):
            ...
            row += 1
            ws[f"A{row}"] = (
                "Recommendation: Maintain current level through regular practice and review"
            )
            ...
            ws[f"A{row}"] = (
                "No domains at 80%+ accuracy yet. Continue focused study on priority areas."
            )
            ws[f"A{row}"].font = Font(italic=True, size=10)
            ws[f"A{row}"].alignment = Alignment(horizontal="left", wrap_text=True)
            row += 1
